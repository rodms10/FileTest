    ...
    exports.getFile = function(successCallback, errorCallback, args) {
        ...
        idb_.get(path.storagePath, function(fileEntry) {
            if (options.create === true && options.exclusive === true && fileEntry) {
                ...
                // FileEntry.
                var newFileEntry = new FileEntry(path.fileName, path.fullPath, fs_);
    ...
    exports.getDirectory = function(successCallback, errorCallback, args) {
        ...
        idb_.get(path.storagePath, function(folderEntry) {
            ...
            if (options.create === true && options.exclusive === true && folderEntry) {
                ...
                // MyDirectoryEntry.
                var dirEntry = new DirectoryEntry(path.fileName, path.fullPath, fs_);
        ...
        if (path.indexOf(pathsPrefix.dataDirectory) === 0) {
            ...
            exports.requestFileSystem(function() {
                exports.getFile(successCallback, function() {
                    exports.getDirectory(successCallback, errorCallback, [null, path, {create: false}]);
                }, [path, null, {create: false}]);
            }, errorCallback, [LocalFileSystem.PERSISTENT]);
        } else if (path.indexOf(pathsPrefix.applicationDirectory) === 0) {
            path = path.substring(pathsPrefix.applicationDirectory.length - 1);

            var xhr = new XMLHttpRequest();
            xhr.open("GET", path, true);
            xhr.onreadystatechange = function () {
                if (xhr.status === 200 && xhr.readyState === 4) {
                    fs_.root.getFile(path, {create: true}, writeFile, function (error) {
        ...
        function writeFile(entry) {
            entry.createWriter(function (fileWriter) {
                fileWriter.onwriteend = function (evt) {
                    if (!evt.target.error) {
                        entry.filesystemName = "application";
